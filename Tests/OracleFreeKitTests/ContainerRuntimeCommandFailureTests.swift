...
struct ContainerRuntimeCommandFailureTests {
    ...
    @Test func commandFailureExplainsRuntimeConnectionFailures() {
        let failure = ContainerRuntimeCommandFailure(
            runtimeName: "Podman",
            operation: .listContainers,
            message: "Cannot connect to Podman. Please verify your connection to the Linux system."
        )

        #expect(failure.errorDescription == "Unable to connect to Podman. Make sure Podman is running, then try again.")
    }
